...
from selfdrive.car.hyundai.values import CAR, Buttons, FEATURES
...
class CarInterface(CarInterfaceBase):
  ...
  def get_pid_accel_limits(CP, current_speed, cruise_speed):
    ...
    gas_max_v = [1.3, 1.2, 1., 0.8, 0.55, 0.45, 0.33, 0.15]

    brake_max_bp = [0, 5., 10., 25., 50., 70., 130.]
    brake_max_v = [-1.5, -2., -2.8, -3.3, -5., -4., -2]
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    if lat_control_method == 0:
      ...
      ret.lateralTuning.pid.kpV = [0.055, 0.06, 0.06]
      ret.lateralTuning.pid.kiBP = [0., 30.]
      ret.lateralTuning.pid.kiV = [0.02, 0.03]
      ret.lateralTuning.pid.kdBP = [0.]
      ret.lateralTuning.pid.kdV = [1.3]

      ret.steerActuatorDelay = 0.1
      ret.steerRateCost = 0.58
    ...
    ret.startAccel = -0.8
    ret.stopAccel = -2.0
    ret.startingAccelRate = 3.4  # brake_travel/s while releasing on restart
